...
export function findPath(previous, endNode, nodeIdIdxMap) {
    if (endNode === undefined)
        return;
    const path = [];
    let u = String(nodeIdIdxMap.get(endNode.id));
    if (u === undefined)
...
export function dijkstra(startNode, nodeIdIdxMap) {
    if (!startNode)
        return;
    let distances = {};
    let previous = {};
...
export function buildGraph(ways, nodes, nodeIdIdxMap) {
    const graph = new Map();
    let edges = 0;
